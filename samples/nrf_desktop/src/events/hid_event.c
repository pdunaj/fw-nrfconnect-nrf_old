...
static int log_args_mouse_xy(struct log_event_buf *buf, const struct event_header *eh)
{
...
static int log_args_mouse_wp(struct log_event_buf *buf, const struct event_header *eh)
{
...
EVENT_INFO_DEFINE(hid_mouse_xy_event, ENCODE(PROFILER_ARG_S32, PROFILER_ARG_S32), ENCODE("dx", "dy"), log_args_mouse_xy);
EVENT_TYPE_DEFINE(hid_mouse_xy_event, print_event, &hid_mouse_xy_event_info);

EVENT_INFO_DEFINE(hid_mouse_wp_event, ENCODE(PROFILER_ARG_S32, PROFILER_ARG_S32), ENCODE("wheel", "pan"), log_args_mouse_wp);
