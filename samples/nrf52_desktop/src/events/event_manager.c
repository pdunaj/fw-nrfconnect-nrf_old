...
static sys_dlist_t eventq = {.head = &eventq, .tail = &eventq };
...
static void event_processor_fn(struct k_work *work)
{
	...
	while (NULL != (eh = CONTAINER_OF(sys_dlist_get(&events),
		...
		/* TODO We can hook the event to sysview here */

		/* TODO this requires header to be the first element in the
		 * structure.
		 */
		k_free(eh);
	}
...
{
	/* TODO: This looks nice but we won't be able to put events from the
	 * interrupts!
	 * we may disable interrupts - there are quick operations on both sides
	 * we may trigger each event in separate work (seems overkill)
	 * we may go lockless with atomics and ring buffer ( I guess this seems
	 * the best option)
	 */
	unsigned int flags = irq_lock();
