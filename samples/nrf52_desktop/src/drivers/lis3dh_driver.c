...
#define MODULE		lis3dh_driver
#define MODULE_NAME	STRINGIFY(MODULE)

#define SYS_LOG_DOMAIN	MODULE_NAME
#define SYS_LOG_LEVEL	CONFIG_DESKTOP_SYS_LOG_LIS3DH_DRIVER_LEVEL
...
K_WORK_DEFINE(lis3dh_driver_async_init, async_init_fn);
...
static bool event_handler(const struct event_header *eh)
{
	if (is_module_event(eh)) {
		...
		if (check_state(event, "board_driver", "ready")) {
			if (!active) {
				active = true;
				k_work_submit(&lis3dh_driver_async_init);
			}
			return false;
		}
