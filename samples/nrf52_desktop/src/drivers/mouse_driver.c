...
#define MODULE		mouse_driver
#define MODULE_NAME	STRINGIFY(MODULE)

#define SYS_LOG_DOMAIN	MODULE_NAME
#define SYS_LOG_LEVEL	CONFIG_DESKTOP_SYS_LOG_MOUSE_DRIVER_LEVEL
...
static struct k_delayed_work mouse_driver_scan;
...
static void scan_fn(struct k_work *work)
{
	...
	if (err) {
		...
		if (active) {
			k_delayed_work_submit(&mouse_driver_scan, 50);
		}
	}
}
...
static void async_init_fn(struct k_work *work)
{
	...
	k_delayed_work_submit(&mouse_driver_scan, 25);
}
K_WORK_DEFINE(mouse_driver_async_init, async_init_fn);
...
static void async_term_fn(struct k_work *work)
{
	...
	k_delayed_work_cancel(&mouse_driver_scan);
...
K_WORK_DEFINE(mouse_driver_async_term, async_term_fn);
...
static bool event_handler(const struct event_header *eh)
{
	if (is_module_event(eh)) {
		...
		if (check_state(event, "main", "ready")) {
			...
			k_delayed_work_init(&mouse_driver_scan, scan_fn);
			return false;
		}
		...
		if (check_state(event, "board_driver", "ready")) {
			if (!active) {
				active = true;
				k_work_submit(&mouse_driver_async_init);
			}
			return false;
		}
	...
	if (is_power_down_event(eh)) {
		if (!terminating && active) {
			k_work_submit(&mouse_driver_async_term);
			terminating = true;
		}
